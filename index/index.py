...
class Index(AbstractIndex):
    def __init__(self, embeddings: np.array, dimension: int, normalise=False):
        ...
        if(embeddings.shape[1]!=dimension):
            raise ValueError(f"Expected embeddings of dimension {dimension} but got {embeddings.shape[1]}")
        
        self.embeddings = embeddings if not normalise else normalise_embeddings(embeddings)
